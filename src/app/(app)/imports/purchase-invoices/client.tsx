...
import { importPurchaseInvoices } from "@/lib/actions/import";
import { Upload, FileCheck, AlertTriangle, FileText } from "lucide-react";
...
export function PurchaseInvoiceImportClient({ batches }: { batches: ImportBatch[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await importPurchaseInvoices(formData);
      setResult(res);
      router.refresh();
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Colunas esperadas: Número NF, Série, Data Emissão, CNPJ Fornecedor, Nome
                Fornecedor, Valor Total, Desconto, Frete, Chave Acesso
              </p>
            </div>
