...
import { importCardTransactions } from "@/lib/actions/import";
import { Upload, FileCheck, AlertTriangle, CreditCard } from "lucide-react";
...
export function CardTransactionImportClient({ batches }: { batches: ImportBatch[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await importCardTransactions(formData);
      setResult(res);
      router.refresh();
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Colunas esperadas: Data Transação, Descrição, Valor Bruto, Taxa, Valor Líquido,
                Bandeira, Final Cartão, Parcela, Total Parcelas
              </p>
            </div>
